...
def train_one_epoch(model: torch.nn.Module, criterion,
    ...
    for samples, targets in data_loader:
        ...
        if len(sample_outputs) < 10:
            ...
            sample_targets.append(targets.detach().cpu().numpy())
    ...
    # Debug print
    if sample_outputs:
        sample_outputs = np.concatenate(sample_outputs)
        sample_targets = np.concatenate(sample_targets)
        print(f"Train debug - Sample outputs (first 5): {sample_outputs[:5].flatten()}")
        print(f"Train debug - Sample targets (first 5): {sample_targets[:5].flatten()}")
        print(f"Train debug - Output std: {np.std(sample_outputs):.6f}, Target std: {np.std(sample_targets):.6f}")
...
def evaluate(data_loader, model, device, amp_autocast, args, criterion):
    ...
    if args.task == 'classification':
        ...
        test_stats = {'mae': mean_absolute_error(targets_np, pred_np), 'r2': r2_score(targets_np, pred_np), 'r': pearsonr(targets_np, pred_np)[0]}
        
        # Debug: print sample predictions and targets
        print(f"Sample targets (first 5): {targets_np[:5]}")
        print(f"Sample predictions (first 5): {pred_np[:5]}")
        print(f"Pred std: {np.std(pred_np):.6f}, Target std: {np.std(targets_np):.6f}")
