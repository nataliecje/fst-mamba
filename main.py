...
def get_args_parser():
    ...
    parser.add_argument('--drop_rate', type=float, default=0.1, help='dropout rate for FunctionalMamba blocks')
    parser.add_argument('--drop_path_rate', type=float, default=0.1, help='stochastic depth rate')
    parser.add_argument('--early_stop_patience', type=int, default=20, help='epochs to wait for improvement before early stop')
    parser.add_argument('--early_stop_delta', type=float, default=0.0, help='minimum change to qualify as improvement')
...
def main(args):
    ...
    for fold_id in args.fold_ids:
        ...
        # Use img_size=256 for EMOTION and WM datasets, otherwise 64
        img_size = 256 if ('EMOTION' in args.dataset_name or 'WM' in args.dataset_name) else 64
        # Use spatial-only (bi_mamba) for better training stability with timeseries data
        ...
        # Use CosineAnnealingLR with longer warmup for better convergence
        ...
        if args.task == 'classification':
            ...
            best_metric_value = -float('inf')  # maximize auc
        else:
            best_stats = {'mae': (1e6, ), 'r2': (0, ), 'r': (0, )}
            best_metric_value = float('inf')   # minimize mae
        ...
        for e in epochs:
            ...
            if args.task == 'classification':
                print(f"Epoch {e}: Train Loss={train_stats['loss']:.4f}, Test Acc={test_stats.get('acc', 0):.4f}, Test AUC={test_stats.get('auc', 0):.4f}")
            else:
                print(f"Epoch {e}: Train Loss={train_stats['loss']:.4f}, Train Corr={train_stats.get('r', 0):.4f}, Val Corr={test_stats.get('r', 0):.4f}, Val MAE={test_stats.get('mae', 0):.4f}")

            if utils.get_rank() == 0:
                best_stats = utils.save_best(test_stats, best_stats, e, fold_id, out_pred, model, args)

                # Early stopping based on primary metric
                current_metric = test_stats[target_metric]
                improved = False
                if args.task == 'classification':
                    if current_metric > best_metric_value + args.early_stop_delta:
                        best_metric_value = current_metric
                        improved = True
                else:  # regression (mae to minimize)
                    if current_metric < best_metric_value - args.early_stop_delta:
                        best_metric_value = current_metric
                        improved = True
                ...
                if early_stop_counter >= args.early_stop_patience:
                    print(f"Early stopping at epoch {e} (no improvement in {args.early_stop_patience} epochs)")
                    break
    ...
    with open(os.path.join(preds_root, 'job_status.txt'), 'a') as f:
